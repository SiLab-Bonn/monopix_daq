...
def get_te_offset(dat,debug=None,field="timestamp"):
     hist=np.histogram((dat[field] - (np.uint64(dat["te"])<<np.uint64(4)) ) & 0xFF0, bins=np.arange(0,0x1001))
...
def build_h5(fin,fout,debug=0x0):
    ...
    with tables.open_file(fout, "w") as f_o:
        ...
        for i,u in enumerate(uni):
            arg=np.argmin(np.abs(np.int8((u/0x10)-dat["te"][idx[i]:idx[i]+cnt[i]]-(te_offset/0x10))))
