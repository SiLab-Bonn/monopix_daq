    ...
    while d_i < len(dat):
        ...
        if dat[d_i]["col"]==TS_INJ:
            ...
            if d_ii == len(dat) and (mode & 0x1)==1: ## not the end of file:
                return 1,d_i,buf[:b_i],sid,pre_inj,inj_id,inj_cnt
            cnt=d_ii-d_i
            ...
            for d_ii in range(d_i+1, d_i+cnt):
                ...
                elif dat[d_ii]["col"]<COL_SIZE and dat[d_ii]["cnt"]==0:
...
def build_inj_h5(fhit,fraw,fout,n=50,debug=0x2):
    ...
    with tables.open_file(fraw) as f:
        ...
        injlist=yaml.load(f.root.meta_data.attrs.injlist) ###inj is inj_high-inj_low!!!
        thlist=yaml.load(f.root.meta_data.attrs.thlist)
        phaselist=yaml.load(f.root.meta_data.attrs.phaselist)
