...
local_configuration={"phaselist": np.arange(0,16,1),
                     "injlist": np.arange(0.05,1.5,0.05),
                     'pix': [18,25],
                     "with_mon": True
}
    ...
    def plot(self):
        ...
        with plotting_base.PlottingBase(fpdf,save_png=True) as plotting:
            with tb.open_file(fraw) as f:
                ...
                injlist=np.sort(np.unique(np.array(yaml.load(f.root.meta_data.attrs.injlist))))
            ...
            with tb.open_file(fev) as f:
                ...
                ## S-curve
                res,le,ph=get_scurve(f.root,injlist,inj_n)
...
def get_scurve(fhit_root,injlist,inj_n):
