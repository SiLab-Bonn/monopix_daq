...
def init_all(dut=False,hv=True,hv2=True,hv3=True,mso=True):
    if dut==True:
        l=monopix_extensions.MonopixExtensions("monopix.yaml")
    else:
        l=None
    if hv==True:
...
def init_testbeam(dut=False,hv=True,pwr=True,mso=False):
    if dut==True:
        l=lfcpix.lfcpix()
    else:
        l=None
    if hv==True:
        ...
        hv=None
    if pwr==True:
        pwr=AgilentE3646A.AgilentE3646A("/dev/ttyUSB0")
    else:
        pwr=None
    if mso==True:
        mso=MSO4104B_sock.Mso4104_sock('192.168.10.16')
